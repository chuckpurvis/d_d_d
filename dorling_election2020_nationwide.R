########################################################
#  dorling_election2020_nationwide.r
...
temp1_proj <- st_transform(temp1,5070) # Conus Albers, NAD83
...
ggplot() +
  ...
  labs(title = "2020 Presidential Election Results: Contiguous US States",
...
ggsave("nationwide_county_presid_election_2020_2.png", plot=last_plot(),
...
ggplot(data=temp1_proj) +
  ...
  scale_fill_manual(values=c("blue", "red")) +
  theme_void() + 
  geom_sf(data=centers1, alpha=1.0, aes(fill=winner, size=plurality_abs),
          show.legend=FALSE,
  ...
  labs(title = "2020 Presidential Election Results: Contiguous United States",
       subtitle = "Size of Circle is Plurality of Winner in County",
       caption = "Created: CPurvis; Data sources: Census 2020 (via tidycensus), MIT Election Lab",
       fill = "Plurality of Votes by County") + 
  theme(
...
ggsave("nation_county_presid_election_2020_2_bigontop_projected.png", plot=last_plot(),
       dpi=300,width=8,height=7)   
#########################################################################################
