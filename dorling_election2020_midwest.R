...
states1 <- tigris::states(year=2020,cb=TRUE) %>% 
           filter(STUSPS %in% c("OH","MI","IL","IN","WI")) %>% 
           st_transform(3078)
...
temp1_proj <- st_transform(temp1,3078) # Michigan Oblique Mercator, NAD83
...
ggsave("midwest_county_presid_election_2020_2.png", plot=last_plot(),
...
ggsave("midwest_county_presid_election_2020_1_projected.png", plot=last_plot(),
       dpi=300,width=8,height=7)   
#########################################################################################
ggplot(data=temp1_proj) +
  geom_sf(data=temp1_proj, alpha=0.4, aes(fill=winner)) +  #, show.legend=FALSE) + 
...
ggsave("midwest_county_presid_election_2020_0_choropleth_projected.png", plot=last_plot(),
       dpi=300,width=8,height=7)   
#########################################################################################
